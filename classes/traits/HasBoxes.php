...
trait HasBoxes
{
    ...
    /**
     * Apply a Boxes template configuration.
     */
    public function applyBoxesTemplate(string $handle)
    {
        $templates = PartialReader::instance()->getBoxesConfig()->get('templates');

        if ($templates->has($handle) === false) {
            throw new LogicException("[OFFLINE.Boxes] The template '{$handle}' does not exist.");
        }

        $boxes = $templates->get($handle)->get('boxes');

        $this->boxes()->saveMany($boxes->map(fn (array $box) => Box::make($box)));
    }
